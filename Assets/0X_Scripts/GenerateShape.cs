...
public class GenerateShape : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        origin = userTransform.position;
        originalForward = userTransform.forward; //as the spawn point will always be the direction the user is facing
        forward = originalForward;

        originalRandomA = randomA;

        viewpoint = getViewPoint(forward, viewDistance);
    ...
    private void OnDrawGizmos () {
        ...
        Gizmos.DrawSphere(viewpoint, 1);
        Gizmos.DrawLine(origin, viewpoint);


        if (startEnd != null) {
            Gizmos.DrawSphere(startEnd[0], .5f);
            Gizmos.DrawSphere(startEnd[1], .5f);
        }
        if (planePoint != null) {
        ...
        if (middleNodes != null) {
            ...
            foreach(Vector3 node in middleNodes) {
                ...
                Gizmos.DrawCube(node, new Vector3(.5f, .5f, .5f));
                i+=1;
            }
        }
    ...
    {
        origin = userTransform.position;
        forward = userTransform.forward; //as the spawn point will always be the direction the user is facing
    ...
    Vector3 getViewPoint(Vector3 forward, float distance) {
        ...
        Vector3 viewpoint = forward * distance;
        return viewpoint;
    }
