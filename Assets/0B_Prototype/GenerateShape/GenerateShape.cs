...
//*** the script only contains functions that can be used as nodes in visual scripting
namespace CMA
{
    public class GenerateShape : MonoBehaviour
    {
        // generates architecture in 1 DIRECTION base on single user position and target_position
        // target_position will be retrieved from headset focus point

        //all public values can be easily and dynamically changed as the experience progresses
        public float linePercent = 0.7f;
        public float originalLinePercent;

        //window parameters
        public float middle_position = 0.5f; //ratio factor viewer-middle window / viewer-target
        public float middle_height = 30f;
        public float middle_widthMult = 1.5f;
        public int middle_division = 4; //the rect which will be divided into diamonds has proportions y x y*this value

        public float inward_position = 0.1f;    //the ratio factor viewer-inward window / viewer-middle window

        public float outward_position = 0.1f;   //the ratio factor target-outward window / target-middle window

        public float randomA = .05f;
        float originalRandomA;
        float[] randomsA;

        // amount of randomness compared to line length from user to target_position for A diamonds
        // so if == 0.05f, diamond extrusion can vary by as much as 5% of the total line length
        // could be change to be a percent of just the range of extrusion (so viewer_position - plane for diamonds A)

        public Material tubeMaterial;

        private Vector3 viewer_position;
        private Vector3 target_position;
        private Vector3[] startEnd;
        private Vector3 planePoint;
        private Vector3[] rectOutline;
        private Vector3[,] crossHatch;

        private Vector3[] diamondNodes;
        private Vector3[] middleNodes;

        private Vector3[,] diamondsA;
        private Vector3[,] diamondsMirror;
        private Vector3[,] diamondsMiddle;
        ...
        {
            viewer_position = GetComponent<GazeInteractable>().GetGazeInteractor().transform.position;
            target_position = getViewPoint();

            originalRandomA = randomA;

            startEnd = getPointRange(viewer_position, target_position, linePercent);
            planePoint = getPlaneOrigin(startEnd);
            rectOutline = getPlaneOutline(middle_height, middle_widthMult, planePoint, viewer_position);
            crossHatch = getCrossHatch(rectOutline, middle_division, middle_height, middle_widthMult);
            diamondNodes = getDiamondNodes(rectOutline, middle_division);
            middleNodes = getMiddleNodes(rectOutline, middle_division);
            middleNodes = sortMiddleNodes(middleNodes, rectOutline);

            diamondsA = createFirstDiamonds(middleNodes, startEnd, planePoint,
                                        middle_division, middle_height, middle_widthMult, rectOutline);
            diamondsMirror = createFirstDiamonds(middleNodes, startEnd, planePoint,
                                        middle_division, middle_height, middle_widthMult, rectOutline, true);
            diamondsMiddle = createMiddleDiamonds(middleNodes, middle_height, middle_widthMult, rectOutline,
                                        middle_division, planePoint);
        ...
        //     Gizmos.DrawSphere(viewer_position, .2f);
        //     Gizmos.DrawSphere(target_position, .2f);
        //     Gizmos.DrawLine(viewer_position, target_position);
        ...
        //     }
        //     if (crossHatch != null) {
        //         for (int i=0; i<middle_division*middle_division*2; i++) {
        ...
        Vector3 getViewPoint() {
            ...
            target_position = GetComponent<GazeInteractable>().GetGazeInteractor().getGazePoint();
            return target_position;
        }

        Vector3[] getPointRange(Vector3 viewer_position, Vector3 target_position, float linePercent) {
            // takes viewer_position, target_position, and how much of the line we can choose from,
            // then generates range where the field can originate from
            // right now the range is equidistant from the viewer_position to the target_position
            Vector3 line = target_position - viewer_position;
            Vector3 start = (line * ((1-linePercent)/2)) + viewer_position;
        ...
        Vector3[] getPlaneOutline(float middle_height, float middle_widthMult, Vector3 planeOrigin, Vector3 viewer_position) {
            Vector3 lineVector = planeOrigin - viewer_position;
            Vector3 normal = Vector3.Cross(lineVector, Vector3.up).normalized; //get perpendicular line
            float width = middle_widthMult * middle_height;
            ...
            Vector3 upLeft = (middle_height/2) * normal90 + left;
            Vector3 upRight = (middle_height/2) * normal90 + right;
            Vector3 downLeft = left - (middle_height/2) * normal90;
            Vector3 downRight = right - (middle_height/2) * normal90;
            return new [] {downLeft, downRight, upLeft, upRight};
        }

        Vector3[,] getCrossHatch(Vector3[] corners, int divs, float middle_height, float middle_widthMult) {
            float width = middle_height * middle_widthMult;
            float widthSection = width / divs;
            float section = middle_height / divs;
        ...
        Vector3[,] createFirstDiamonds(Vector3[] sortedMiddles, Vector3[] startEnd, Vector3 planeOrigin,
                                        int divs, float middle_height, float middle_widthMult, Vector3[] corners, bool mirror=false) {
            ...
            Vector3 diamondHeightHalf = ((middle_height/divs)/2) * height; //actually these should be scaled
            Vector3 diamondWidthHalf = (((middle_height*middle_widthMult)/divs)/2) * width; //actually these should be scaled
        ...
        Vector3[,] createMiddleDiamonds(Vector3[] sortedMiddles, float middle_height, float middle_widthMult, Vector3[] corners,
            ...
            for (int i = 0; i<numDiamonds; i++) {
                // Vector3 diamondOrigin = (perc_i * wholeRange) + randomsA[i]*wholeRange.normalized +  sortedMiddles[i];
                Vector3 diamondOrigin = sortedMiddles[i] - planeOrigin; //almost, just dont multiply in the direction of gaze
                Vector3 left = diamondOrigin - diamondWidthHalf;
                Vector3 right = diamondOrigin + diamondWidthHalf;
                Vector3 top = diamondOrigin + diamondHeightHalf;
                Vector3 bottom = diamondOrigin -diamondHeightHalf;
        ...
        //                                 int divs, float middle_height, float middle_widthMult, Vector3[] corners) {
        ...
        void doubleSideCollider(GameObject tube)
        {
            ...
            tube.GetComponent<MeshCollider>().material = tubePhysics;  
            ...
            GameObject reversedMesh = new GameObject("reversedMesh");
            reversedMesh.transform.SetParent(tube.transform);
            reversedMesh.AddComponent<MeshCollider>().sharedMesh = mesh2;
            reversedMesh.GetComponent<MeshCollider>().material = tubePhysics;
